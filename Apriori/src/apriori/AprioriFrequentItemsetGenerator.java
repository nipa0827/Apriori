...
public class AprioriFrequentItemsetGenerator<I> {
	...
	private List<Set<I>> generateCandidates(List<Set<I>> itemsetList) {
		...
		for (Set<I> itemset : itemsetList) {
			List<I> templist = new ArrayList<>(itemset);
			
			Collections.<I>sort(templist, ITEM_COMPARATOR);
	...
	private Set<I> tryMergeItemsets(List<I> itemset1, List<I> itemset2) {
		...
		//System.out.println("itmst "+itemset1+" "+itemset2);
		for (int i = 0; i < length - 1; ++i) {
			if (!itemset1.get(i).equals(itemset2.get(i))) {//if no prefix matching return
				return null;
			}
		}

		if (itemset1.get(length - 1).equals(itemset2.get(length - 1))) {//end same and match prefix all same return
			return null;
		}
		...
		ret.add(itemset1.get(length - 1));//for one item
	...
	private static final Comparator ITEM_COMPARATOR = new Comparator() {
		...
		public int compare(Object o1, Object o2) {
			return ((Comparable) o1).compareTo(o2);//sort according to string
		}
	...
	private List<Set<I>> findFrequentItems(List<Set<I>> itemsetList, Map<Set<I>, Integer> supportCountMap,
		...
		//System.out.println(supportCountMap);
